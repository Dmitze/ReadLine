...
const getAdminMenuKeyboard = (pendingReviews = 0, pendingFeedback = 0) => {
    ...
    return telegraf_1.Markup.inlineKeyboard([
        [
            ...
            telegraf_1.Markup.button.callback('🎙️ Підкаст', 'add_podcast'),
        ],
        [telegraf_1.Markup.button.callback('📚 Редагування', 'manage_books')],
        [telegraf_1.Markup.button.callback('✨ Деталі', 'manage_extended_book_info')],
        [telegraf_1.Markup.button.callback('📋 Замовлення', 'admin_orders')],
        [telegraf_1.Markup.button.callback('🎁 Промокоди', 'manage_promo_codes')],
...
const getGenresKeyboard = (genres) => {
    ...
    return telegraf_1.Markup.keyboard(keyboard).oneTime().resize().reply_markup;
};
...
const getFeedbackActionKeyboard = (feedbackId, userId) => {
    return telegraf_1.Markup.inlineKeyboard([
        ...
        [telegraf_1.Markup.button.url('👤 Профіль', `tg://user?id=${userId}`)],
    ]).reply_markup;
};
