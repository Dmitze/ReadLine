...
exports.registerPhysicalBooksHandlers = registerPhysicalBooksHandlers;
const telegraf_1 = require("telegraf");
const logger_1 = require("../../utils/logger");
const constants_1 = require("../../constants");
const physicalBooks_1 = require("../../database/tables/physicalBooks");
function registerPhysicalBooksHandlers(bot) {
    bot.hears(constants_1.BUTTONS.REQUEST_PHYSICAL_BOOK, async (ctx) => {
        try {
            ...
            await ctx.reply('📚 <b>ЗАМОВЛЕННЯ ФІЗИЧНОЇ КНИГИ</b>\n\n' +
                ...
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('➕ Створити заявку', 'create_physical_request')],
                    [telegraf_1.Markup.button.callback('📋 Мої заявки', 'my_physical_requests')],
                    [telegraf_1.Markup.button.callback('ℹ️ Правила', 'physical_books_rules')],
                ]).reply_markup,
            });
            logger_1.logger.userAction(userId, 'view_physical_books_menu');
        }
        catch (error) {
            logger_1.logger.error('Error showing physical books menu', error, { userId: ctx.from?.id });
            await ctx.reply(constants_1.ERRORS.GENERIC);
        }
    });
    bot.action('create_physical_request', async (ctx) => {
        await ctx.answerCbQuery();
        await ctx.scene.enter('REQUEST_PHYSICAL_BOOK_SCENE');
    });
    bot.action('my_physical_requests', async (ctx) => {
        try {
            await ctx.answerCbQuery();
            const userId = ctx.from?.id;
            if (!userId) {
                await ctx.editMessageText(constants_1.ERRORS.USER_NOT_FOUND);
                return;
            }
            const requests = await (0, physicalBooks_1.getUserRequests)(userId);
            if (requests.length === 0) {
                await ctx.editMessageText('📋 <b>МОЇ ЗАЯВКИ</b>\n\n' +
            ...
            requests.slice(0, 10).forEach((request) => {
                ...
                keyboard.push([
                    telegraf_1.Markup.button.callback(`${statusEmoji} #${request.id} - ${request.book_title}`, `view_request_${request.id}`),
                ]);
            });
            if (requests.length > 10) {
                message += `<i>Показано 10 останніх заявок з ${requests.length}</i>\n\n`;
            }
            keyboard.push([telegraf_1.Markup.button.callback('➕ Створити нову заявку', 'create_physical_request')]);
            keyboard.push([telegraf_1.Markup.button.callback('🏠 На головну', 'home')]);
    ...
    bot.action(/view_request_(\d+)/, async (ctx) => {
        try {
            ...
            const userId = ctx.from?.id;
            if (!userId) {
                await ctx.answerCbQuery(constants_1.ERRORS.USER_NOT_FOUND, { show_alert: true });
                return;
            }
            const request = await (0, physicalBooks_1.getRequestById)(requestId);
            if (!request) {
            ...
            keyboard.push([telegraf_1.Markup.button.callback('⬅️ Назад до списку', 'my_physical_requests')]);
            keyboard.push([telegraf_1.Markup.button.callback('🏠 На головну', 'home')]);
            await ctx.editMessageText(message, {
                parse_mode: 'HTML',
                reply_markup: telegraf_1.Markup.inlineKeyboard(keyboard).reply_markup,
            });
            logger_1.logger.userAction(userId, 'view_request_details', { requestId });
        }
        catch (error) {
            logger_1.logger.error('Error showing request details', error, {
                userId: ctx.from?.id,
                match: ctx.match,
            });
            await ctx.answerCbQuery('❌ Помилка');
    ...
    bot.action(/cancel_request_(\d+)/, async (ctx) => {
        try {
            const match = ctx.match;
            if (!match)
                return;
            ...
            await ctx.editMessageText('❓ <b>СКАСУВАННЯ ЗАЯВКИ</b>\n\n' +
                '━━━━━━━━━━━━━━━━━━━\n\n' +
                ...
                parse_mode: 'HTML',
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('✅ Так, скасувати', `confirm_cancel_${requestId}`)],
                    [telegraf_1.Markup.button.callback('❌ Ні, залишити', `view_request_${requestId}`)],
                ]).reply_markup,
            });
            await ctx.answerCbQuery();
        }
        catch (error) {
            logger_1.logger.error('Error showing cancel confirmation', error, {
                userId: ctx.from?.id,
                match: ctx.match,
            });
            await ctx.answerCbQuery('❌ Помилка');
    ...
    bot.action(/confirm_cancel_(\d+)/, async (ctx) => {
        try {
            ...
            const cancelled = await (0, physicalBooks_1.cancelRequest)(requestId, userId);
            if (!cancelled) {
                await ctx.editMessageText('❌ Не вдалося скасувати заявку. Можливо, вона вже розглянута.');
                return;
            }
            await ctx.editMessageText('✅ <b>ЗАЯВКУ СКАСОВАНО</b>\n\n' +
                '━━━━━━━━━━━━━━━━━━━\n\n' +
                `📋 Заявка #${requestId} успішно скасована.\n\n` +
                'Ви можете створити нову заявку в будь-який момент.', {
                parse_mode: 'HTML',
                reply_markup: telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('➕ Створити нову заявку', 'create_physical_request')],
                    [telegraf_1.Markup.button.callback('📋 Мої заявки', 'my_physical_requests')],
                    [telegraf_1.Markup.button.callback('🏠 На головну', 'home')],
                ]).reply_markup,
            });
            logger_1.logger.userAction(userId, 'cancel_request', { requestId });
        }
        catch (error) {
    ...
    bot.action('physical_books_rules', async (ctx) => {
        await ctx.answerCbQuery();
        await ctx.editMessageText('📜 <b>ПРАВИЛА КОРИСТУВАННЯ ФІЗИЧНИМИ КНИГАМИ</b>\n\n' +
            '━━━━━━━━━━━━━━━━━━━\n\n' +
            ...
            reply_markup: telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('➕ Створити заявку', 'create_physical_request')],
                [telegraf_1.Markup.button.callback('⬅️ Назад', 'create_physical_request')],
            ]).reply_markup,
        });
    });
}
