...
function registerProfileHandlers(bot) {
    bot.action('back_to_profile_from_orders', async (ctx) => {
    ...
    bot.action(/personal_page_(\d+)/, async (ctx) => {
        await ctx.answerCbQuery();
        const page = parseInt(ctx.match?.[1] || '0', 10);
        const allBooks = ctx.session?.personalCollectionBooks || [];
    ...
    bot.action('back_to_profile_from_personal', async (ctx) => {
        await ctx.answerCbQuery();
        const userId = ctx.from?.id;
        if (!userId)
            return;
    ...
    bot.action('start_ai_assistant', async (ctx) => {
    ...
    bot.action('show_my_orders', async (ctx) => {
        ...
        try {
            ...
            logger_1.logger.error('Error fetching user orders', error instanceof Error ? error : new Error(String(error)));
            await ctx.reply('❌ Помилка при завантаженні замовлень.');
        }
    });
    bot.action('show_personal_collection', async (ctx) => {
        ...
        try {
            ...
            ctx.session.personalCollectionBooks = collectionData.books;
            logger_1.logger.userAction(userId, 'ai_personal_collection', { booksFound: collectionData.count });
        }
        catch (error) {
            logger_1.logger.error('Error in show_personal_collection', error instanceof Error ? error : new Error(String(error)));
            await ctx.reply('❌ Помилка при створенні персональної підбірки.');
        }
    });
}
