...
searchScene.on('text', async (ctx) => {
    ...
    if (searchType === 'title') {
        ...
        books = await new Promise((resolve, reject) => {
            db.all('SELECT * FROM books WHERE title LIKE ? ORDER BY rating DESC LIMIT ?', [`%${searchTerm}%`, SEARCH_LIMIT], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
        ...
        books = await new Promise((resolve, reject) => {
            db.all('SELECT * FROM books WHERE author LIKE ? ORDER BY rating DESC LIMIT ?', [`%${searchTerm}%`, SEARCH_LIMIT], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
        ...
        books = await new Promise((resolve, reject) => {
            db.all('SELECT * FROM books WHERE genre LIKE ? ORDER BY rating DESC LIMIT ?', [`%${searchTerm}%`, SEARCH_LIMIT], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows || []);
