...
const createBookRequestScene = new telegraf_1.Scenes.BaseScene('CREATE_BOOK_REQUEST_SCENE');
createBookRequestScene.enter(async (ctx) => {
    const state = ctx.scene.state;
    state.step = 'title';
    await ctx.reply('📚 <b>НОВА ЗАЯВКА НА КНИГУ</b>\n\n' +
        '━━━━━━━━━━━━━━━━━━━\n\n' +
    ...
    logger_1.logger.userAction(ctx.from?.id || 0, 'start_create_request');
});
createBookRequestScene.on('text', async (ctx) => {
    const state = ctx.scene.state;
    ...
    if (state.step === 'title') {
        ...
        await ctx.reply('✅ Назва збережена!\n\n' +
            ...
            '💡 <i>Приклад: "Тарас Шевченко"</i>', { parse_mode: 'HTML' });
        return;
    }
    ...
    if (state.step === 'genre') {
        ...
        state.step = 'comment';
        await ctx.reply('✅ Жанр збережено!\n\n' +
            '━━━━━━━━━━━━━━━━━━━\n\n' +
...
async function showConfirmation(ctx, state) {
    ...
    await ctx.reply(message, {
        ...
        reply_markup: telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('✅ Підтвердити', 'confirm_create_request')],
            [telegraf_1.Markup.button.callback('❌ Скасувати', 'cancel_create_request')],
        ]).reply_markup,
    });
    state.step = 'confirmation';
}
createBookRequestScene.action('confirm_create_request', async (ctx) => {
    try {
        ...
        const requestId = await (0, bookRequests_1.createBookRequest)({
            user_id: userId,
            book_title: state.book_title,
            book_author: state.book_author,
            book_genre: state.book_genre,
            comment: state.comment,
            status: bookRequests_1.BookRequestStatus.PENDING,
            priority: bookRequests_1.BookRequestPriority.MEDIUM,
        });
        await ctx.editMessageText('✅ <b>ЗАЯВКУ СТВОРЕНО!</b>\n\n' +
        ...
        logger_1.logger.userAction(userId, 'create_request', { requestId, title: state.book_title });
        ...
        logger_1.logger.error('Error creating request', error, { userId: ctx.from?.id });
        await ctx.reply('❌ Помилка при створенні заявки. Спробуйте пізніше.');
        return ctx.scene.leave();
    }
});
createBookRequestScene.action('cancel_create_request', async (ctx) => {
...
exports.default = createBookRequestScene;
//# sourceMappingURL=createBookRequestScene.js.map