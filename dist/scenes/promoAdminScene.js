...
const promoAdminScene = new telegraf_1.Scenes.BaseScene('PROMO_ADMIN_SCENE');
promoAdminScene.enter(async (ctx) => {
    try {
        const stats = await (0, promoCodeFunctions_1.getExtendedPromoStats)();
        await ctx.reply('🎁 <b>КЕРУВАННЯ ПРОМОКОДАМИ</b>\n\n' +
            '📊 <b>Статистика:</b>\n' +
        ...
        logger_1.logger.error('Error loading promo stats on enter', error);
        await ctx.reply('❌ Помилка при завантаженні статистики');
    }
});
promoAdminScene.action('promo_add', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.editMessageText('🎫 <b>ДОДАВАННЯ НОВОГО ПРОМОКОДУ</b>\n\n' +
        ...
        '✨ <b>Система автоматично визначить тип та розмір знижки!</b>', { parse_mode: 'HTML' });
    ctx.scene.state.waitingForPromoCode = true;
});
promoAdminScene.action('promo_list', async (ctx) => {
    await ctx.answerCbQuery('Завантаження...');
    try {
        const promoCodes = await (0, promoCodeFunctions_1.getAllPromoCodes)();
        if (promoCodes.length === 0) {
            await ctx.editMessageText('📋 <b>СПИСОК ПРОМОКОДІВ</b>\n\n' +
                '📭 Промокодів ще немає.\n\n' +
        ...
        for (const promo of promoCodes.slice(0, limits_1.LIMITS.DISPLAY_LIMIT)) {
        ...
        logger_1.logger.error('Error loading promo codes list', error);
...
promoAdminScene.action('promo_stats', async (ctx) => {
    ...
    try {
        ...
        logger_1.logger.error('Error loading extended promo stats', error);
...
promoAdminScene.on('text', async (ctx) => {
    if (!ctx.scene.state.waitingForPromoCode) {
        return;
    }
    ...
    try {
        const existing = await (0, promoCodeFunctions_1.getPromoCodeByCode)(code);
        if (existing) {
            await ctx.reply('❌ *Промокод вже існує!*\n\n' +
                `Код \`${code}\` вже додано раніше.\n` +
                'Спробуйте інший код.', { parse_mode: 'Markdown' });
            return;
        }
        const promoId = await (0, promoCodeFunctions_1.addPromoCode)(code, ctx.from?.id);
        const newPromo = await (0, promoCodeFunctions_1.getPromoCodeByCode)(code);
        if (!newPromo) {
            await ctx.reply('❌ Помилка при отриманні даних промокоду');
            return;
        }
        await ctx.reply('✅ *ПРОМОКОД УСПІШНО ДОДАНИЙ!*\n\n' +
            `🎫 *Код:* \`${newPromo.code}\`\n` +
            ...
            '🔄 Після використання промокод можна повернути, і він стане доступним знову.', { parse_mode: 'Markdown' });
        logger_1.logger.adminAction(ctx.from?.id || 0, 'add_promo_code', { code, promoId });
        ctx.scene.state.waitingForPromoCode = false;
        setTimeout(async () => {
            await ctx.scene.enter('PROMO_ADMIN_SCENE');
        }, 2000);
    }
    catch (error) {
        logger_1.logger.error('Error adding promo code', error);
...
promoAdminScene.leave((ctx) => {
    const state = ctx.scene.state;
    if (state) {
        delete state.waitingForPromoCode;
    }
    logger_1.logger.debug('PromoAdminScene cleanup completed', { userId: ctx.from?.id });
});
exports.default = promoAdminScene;
//# sourceMappingURL=promoAdminScene.js.map