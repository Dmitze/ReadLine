...
const BookManagementService_1 = require("../services/BookManagementService");
const models_2 = require("../database/models");
const mainKeyboards_1 = require("../keyboards/mainKeyboards");
const manageBooksScene = new telegraf_1.Scenes.BaseScene('MANAGE_BOOKS_SCENE');
manageBooksScene.enter(async (ctx) => {
    const books = await (0, models_1.getAllBooks)();
    if (books.length === 0) {
        await ctx.reply('📭 В каталозі немає книг');
        return ctx.scene.leave();
    }
    await ctx.reply('📚 *Управління книгами*\n\n' + `Всього книг: ${books.length}\n\n` + 'Оберіть спосіб пошуку:', {
        parse_mode: 'Markdown',
        reply_markup: telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('📚 За жанром', 'filter_by_genre')],
            [telegraf_1.Markup.button.callback('🔍 Пошук за назвою', 'search_by_title')],
            [telegraf_1.Markup.button.callback('📋 Показати всі (перші 10)', 'show_all_books')],
            [telegraf_1.Markup.button.callback('✏️ Масове редагування', 'bulk_edit')],
            [telegraf_1.Markup.button.callback('⬅️ Назад', 'back_to_admin')],
        ]).reply_markup,
    });
});
manageBooksScene.action('filter_by_genre', async (ctx) => {
    await ctx.answerCbQuery();
    const { getGenres } = await Promise.resolve().then(() => __importStar(require('../database/models')));
    ...
    const keyboard = genres.map((genre, index) => [
        telegraf_1.Markup.button.callback(genre, `genre_filter_${index}`),
    ]);
    keyboard.push([telegraf_1.Markup.button.callback('⬅️ Назад', 'back_to_manage')]);
    await ctx.editMessageText('📚 *Оберіть жанр:*', {
        parse_mode: 'Markdown',
        reply_markup: telegraf_1.Markup.inlineKeyboard(keyboard).reply_markup,
    });
});
manageBooksScene.action(/genre_filter_(\d+)/, async (ctx) => {
...
⭐ ${book.rating || 0}/5
    `.trim();
        await ctx.reply(bookText, {
            ...
            reply_markup: telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('✏️ Редагувати', `edit_book_${book.id}`),
                    telegraf_1.Markup.button.callback('🗑️ Видалити', `delete_book_${book.id}`),
                ],
            ]).reply_markup,
        });
    }
...
manageBooksScene.action('search_by_title', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.editMessageText('🔍 *Пошук за назвою*\n\n' +
        'Введіть назву книги або частину назви:\n\n' +
        'Наприклад: `/find Кобзар`', { parse_mode: 'Markdown' });
});
manageBooksScene.action('show_all_books', async (ctx) => {
...
manageBooksScene.action('back_to_manage', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.scene.reenter();
});
manageBooksScene.action('back_to_admin', async (ctx) => {
    await ctx.answerCbQuery();
...
manageBooksScene.action(/edit_book_(\d+)/, async (ctx) => {
    const match = ctx.match;
    if (!match || !match[1]) {
        logger_1.logger.error('Edit book: no bookId in match', { match, userId: ctx.from?.id });
        await ctx.answerCbQuery('❌ Помилка: не вдалося отримати ID книги');
        return;
    }
    const bookId = parseInt(match[1]);
    logger_1.logger.info('Edit book action triggered', { bookId, userId: ctx.from?.id });
    await ctx.answerCbQuery('Відкриваємо редагування...');
    logger_1.logger.info('Entering EDIT_BOOK_SCENE', { bookId, userId: ctx.from?.id });
    await ctx.scene.enter('EDIT_BOOK_SCENE', { bookId });
});
manageBooksScene.action(/delete_book_(\d+)/, async (ctx) => {
    const match = ctx.match;
    if (!match || !match[1]) {
        await ctx.answerCbQuery('❌ Помилка: не вдалося отримати ID книги');
        return;
    ...
    await ctx.answerCbQuery();
    await ctx.reply('⚠️ <b>ПІДТВЕРДЖЕННЯ ВИДАЛЕННЯ</b>\n\n' +
        ...
        parse_mode: 'HTML',
        reply_markup: telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('✅ Так, видалити', `confirm_delete_${bookId}`)],
            [telegraf_1.Markup.button.callback('❌ Ні, залишити', 'cancel_delete')],
        ]).reply_markup,
    });
});
manageBooksScene.action(/confirm_delete_(\d+)/, async (ctx) => {
    const match = ctx.match;
    if (!match || !match[1]) {
        await ctx.answerCbQuery('❌ Помилка: не вдалося отримати ID книги');
        return;
...
manageBooksScene.command('find', async (ctx) => {
    ...
    const { searchBooks } = await Promise.resolve().then(() => __importStar(require('../database/models')));
...
manageBooksScene.action('bulk_edit', async (ctx) => {
    await ctx.answerCbQuery();
    const books = await (0, models_1.getAllBooks)();
    if (books.length === 0) {
    ...
    const bookButtons = books
        .slice(0, 10)
        .map((book) => [
        telegraf_1.Markup.button.callback(`☐ ${book.title} (${book.author})`, `bulk_select_${book.id}`),
    ]);
    bookButtons.push([
        telegraf_1.Markup.button.callback('✅ Готово (0 обрано)', 'bulk_edit_actions'),
        telegraf_1.Markup.button.callback('❌ Скасувати', 'back_to_admin'),
    ]);
    await ctx.reply('📚 *МАСОВЕ РЕДАГУВАННЯ*\n\n' + 'Оберіть книги для редагування:\n' + '(показано перші 10 книг)', {
        parse_mode: 'Markdown',
        reply_markup: telegraf_1.Markup.inlineKeyboard(bookButtons).reply_markup,
    });
});
manageBooksScene.action(/bulk_select_(\d+)/, async (ctx) => {
    ...
    const bookButtons = books.slice(0, 10).map((book) => {
        ...
        return [
            telegraf_1.Markup.button.callback(`${isSelected ? '☑' : '☐'} ${book.title} (${book.author})`, `bulk_select_${book.id}`),
        ];
    });
    bookButtons.push([
        telegraf_1.Markup.button.callback(`✅ Готово (${state.selectedBooks.length} обрано)`, 'bulk_edit_actions'),
        telegraf_1.Markup.button.callback('❌ Скасувати', 'back_to_admin'),
    ]);
    await ctx.editMessageReplyMarkup(telegraf_1.Markup.inlineKeyboard(bookButtons).reply_markup);
});
manageBooksScene.action('bulk_edit_actions', async (ctx) => {
    const state = ctx.scene.state;
    ...
    await ctx.reply('📚 *МАСОВЕ РЕДАГУВАННЯ*\n\n' +
        ...
        parse_mode: 'Markdown',
        reply_markup: telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('✅ Зробити доступними', 'bulk_make_available')],
            [telegraf_1.Markup.button.callback('❌ Зробити недоступними', 'bulk_make_unavailable')],
            [telegraf_1.Markup.button.callback('🏷️ Додати теги', 'bulk_add_tags')],
            [telegraf_1.Markup.button.callback('🗑️ Видалити книги', 'bulk_delete')],
            [telegraf_1.Markup.button.callback('⬅️ Назад', 'bulk_edit')],
        ]).reply_markup,
    });
});
manageBooksScene.action('bulk_make_available', async (ctx) => {
    const state = ctx.scene.state;
    await ctx.answerCbQuery();
    await ctx.reply('⚠️ *ПІДТВЕРДЖЕННЯ*\n\n' +
...
manageBooksScene.action('confirm_bulk_available', async (ctx) => {
    const state = ctx.scene.state;
    await ctx.answerCbQuery('⏳ Оновлюю...');
    const bookService = (0, BookManagementService_1.createBookManagementService)(models_2.db);
    const result = await bookService.bulkUpdateAvailability(state.selectedBooks, true, ctx.from?.id);
    if (result.isOk()) {
        const data = result.unwrap();
        if (data.success) {
            await ctx.editMessageText('✅ *Успішно оновлено!*\n\n' +
                `${data.count} ${data.count === 1 ? 'книга' : 'книг'} тепер доступні`, { parse_mode: 'Markdown' });
        }
        else {
            await ctx.editMessageText('❌ *Помилка при оновленні*\n\n' +
                (data.errors.length > 0 ? data.errors.join('\n') : 'Невідома помилка'), { parse_mode: 'Markdown' });
        }
    }
    else {
...
manageBooksScene.action('bulk_make_unavailable', async (ctx) => {
    const state = ctx.scene.state;
    await ctx.answerCbQuery();
    await ctx.reply('⚠️ *ПІДТВЕРДЖЕННЯ*\n\n' +
...
manageBooksScene.action('confirm_bulk_unavailable', async (ctx) => {
    const state = ctx.scene.state;
    await ctx.answerCbQuery('⏳ Оновлюю...');
    const bookService = (0, BookManagementService_1.createBookManagementService)(models_2.db);
    ...
    if (result.isOk()) {
        ...
        if (data.success) {
            await ctx.editMessageText('✅ *Успішно оновлено!*\n\n' +
                `${data.count} ${data.count === 1 ? 'книга' : 'книг'} тепер недоступні`, { parse_mode: 'Markdown' });
        }
        else {
            await ctx.editMessageText('❌ *Помилка при оновленні*\n\n' +
                (data.errors.length > 0 ? data.errors.join('\n') : 'Невідома помилка'), { parse_mode: 'Markdown' });
...
manageBooksScene.action('bulk_add_tags', async (ctx) => {
    await ctx.answerCbQuery();
    ...
    for (let i = 0; i < allTags.length; i += 2) {
        const row = [telegraf_1.Markup.button.callback(allTags[i].name, `bulk_tag_${allTags[i].id}`)];
        if (i + 1 < allTags.length) {
            row.push(telegraf_1.Markup.button.callback(allTags[i + 1].name, `bulk_tag_${allTags[i + 1].id}`));
        }
        tagButtons.push(row);
    }
    tagButtons.push([
        telegraf_1.Markup.button.callback('✅ Додати теги', 'bulk_apply_tags'),
        telegraf_1.Markup.button.callback('❌ Скасувати', 'bulk_edit_actions'),
    ]);
    await ctx.reply('🏷️ *ДОДАТИ ТЕГИ*\n\n' +
...
manageBooksScene.action(/bulk_tag_(\d+)/, async (ctx) => {
    ...
    for (let i = 0; i < allTags.length; i += 2) {
        ...
        const row = [
            telegraf_1.Markup.button.callback(`${isSelected1 ? '✅ ' : ''}${tag1.name}`, `bulk_tag_${tag1.id}`),
        ];
        if (i + 1 < allTags.length) {
            const tag2 = allTags[i + 1];
            const isSelected2 = state.selectedTags.includes(tag2.id);
            row.push(telegraf_1.Markup.button.callback(`${isSelected2 ? '✅ ' : ''}${tag2.name}`, `bulk_tag_${tag2.id}`));
        }
        tagButtons.push(row);
    }
    tagButtons.push([
...
manageBooksScene.action('bulk_apply_tags', async (ctx) => {
    const state = ctx.scene.state;
    ...
    const { addBookTag } = await Promise.resolve().then(() => __importStar(require('../database/tagFunctions')));
    ...
    await ctx.reply('✅ *Успішно додано!*\n\n' +
        ...
        `${state.selectedBooks.length} ${state.selectedBooks.length === 1 ? 'книги' : 'книг'}`, { parse_mode: 'Markdown' });
    logger_1.logger.info('Bulk update: added tags', {
        adminId: ctx.from?.id,
...
manageBooksScene.leave((ctx) => {
    ...
    logger_1.logger.debug('Manage books scene cleanup completed', { userId: ctx.from?.id });
});
manageBooksScene.action('bulk_delete', async (ctx) => {
    const state = ctx.scene.state;
    if (!state.selectedBooks || state.selectedBooks.length === 0) {
        await ctx.answerCbQuery('⚠️ Оберіть хоча б одну книгу');
        return;
    ...
    await ctx.answerCbQuery();
    const bookNames = await Promise.all(state.selectedBooks.map(async (id) => {
        const book = await (0, models_1.getBookById)(id);
        if (!book) {
            return `• Книга #${id}`;
        }
        return `• ${book.title}`;
    }));
    await ctx.reply('⚠️ *УВАГА! ВИДАЛЕННЯ КНИГ*\n\n' +
...
manageBooksScene.action('confirm_bulk_delete', async (ctx) => {
    const state = ctx.scene.state;
    if (!state.selectedBooks || state.selectedBooks.length === 0) {
    ...
    state.selectedBooks = [];
    await ctx.reply('✅ *ВИДАЛЕННЯ ЗАВЕРШЕНО*\n\n' +
        '📊 Результати:\n' +
        ...
        parse_mode: 'Markdown',
        reply_markup: telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('⬅️ Повернутись до управління', 'back_to_admin')],
        ]).reply_markup,
    });
});
exports.default = manageBooksScene;
//# sourceMappingURL=manageBooksScene.js.map