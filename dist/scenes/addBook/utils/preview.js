...
async function proceedToTags(ctx) {
    ...
    const allTags = await (0, cache_1.getCachedTags)();
    if (allTags.length > 0) {
        const tagButtons = [];
        ...
        tagButtons.push([
            telegraf_1.Markup.button.callback('✅ Далі (без тегів)', `preview_skip_tags_${userId}`),
            telegraf_1.Markup.button.callback('❌ Скасувати', `cancel_add_${userId}`),
        ]);
        if (!state.selectedTags) {
            state.selectedTags = [];
        }
        await ctx.reply(`${(0, progress_1.getProgress)(8)}\n🏷️ <b>Додайте теги до книги (опціонально):</b>\n\n` +
