...
export const getAdaptiveBookKeyboard = (ctx: Context, book: Book, isSaved: boolean = false) => {
  ...
  if (book.rating && book.rating > 0) {
    const buttonText = deviceType === 'mobile' ? `⭐ ${book.rating.toFixed(1)}` : `⭐ ${book.rating.toFixed(1)} Оцінити`;
...
export const getEnhancedBookKeyboard = (book: Book, isSaved: boolean = false) => {
  ...
  // Кнопка оцінки
  if (book.rating && book.rating > 0) {
    actionRow.push(Markup.button.callback(`⭐ ${book.rating.toFixed(1)} Оцінити`, `rate_${book.id}`));
  } else {
