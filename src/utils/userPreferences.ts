...
export const getUserKeyboardPreference = (userId: number): DeviceType => {
  try {
    ...
    logger.error('Error getting keyboard preference', error instanceof Error ? error : new Error(String(error)));
    return 'mobile';
  }
};
...
export const setUserKeyboardPreference = (userId: number, deviceType: DeviceType): boolean => {
  try {
    ...
    logger.error('Error setting keyboard preference', error instanceof Error ? error : new Error(String(error)));
    return false;
  }
};
...
export const ensureKeyboardTypeColumn = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      logger.error('Error ensuring keyboard_type column', error instanceof Error ? error : new Error(String(error)));
      resolve(); // Не блокуємо запуск бота
    }
  });
};
