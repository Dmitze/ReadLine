...
feedbackScene.on('text', async (ctx: BotContext) => {
  try {
    ...
    try {
      const feedbackId = await addFeedbackMessage({
        user_id: userId,
        user_name: userName,
    ...
    logger.error('Error in feedback scene', error instanceof Error ? error : new Error(String(error)), { userId: ctx.from?.id });
    await ctx.reply('❌ Виникла помилка при відправці повідомлення.');
  }
