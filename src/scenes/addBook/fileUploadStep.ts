...
export async function handleFileFormatUpload(
  ctx: BotContext,
  state: WizardState,
  format: 'file' | 'audio' | 'link'
): Promise<boolean> {
  const userId = ctx.from?.id;
  if (!userId) {
  ...
  if (!allowed) {
    await (ctx as any).reply(
      '❌ Занадто багато завантажень. Зачекайте хвилину та спробуйте ще раз.'
    );
