...
import { getFeedbackActionKeyboard } from '../../keyboards/adminKeyboards';
...
export default (bot: Telegraf<BotContext>) => {
  bot.action('view_feedback', async (ctx) => {
    (async () => {
      ...
      logger.info('Feedback messages loaded', {
      ...
      for (const msg of messages) {
        try {
          ...
          logger.error(
            'Error displaying feedback message',
      ...
      logger.error(
        'Error showing feedback messages',
  ...
  bot.action('view_feedback_history', async (ctx) => {
    (async () => {
      ...
      logger.error(
        'Error showing feedback history',
        error instanceof Error ? error : new Error(String(error)),
        { userId: ctx.from?.id }
      );
